...
- **Hubs that must migrate last:** `sidebarPathVisibilityState`/`sidebarVisibilityToggleState`, `paramsState`/`paramsConfigState`, `viewLayersState`, `viewLayersParamsState`, `selectionState`, plus `terracottaColorMapValuesQuery` (injected through a React context provider).
