...
- **Sync surface:** Jotai per-atom helpers (`atomWithUrlSync`, `atomWithLocalStorage`, `RouteParamSync`) replaced Recoil sync for all migrated features. **Downloads job pipeline decommissioned (2026-06-02)** — removed the only `syncEffect({ storeKey: 'local-storage' })` consumers; `<RecoilLocalStorageSync>` unmounted from [`src/App.tsx`](../../../src/App.tsx). `<RecoilURLSyncJSON>` and route-scoped `MapViewRouteSync` are also gone from active Recoil atoms (URL/route sync is on Jotai); both Recoil sync wrappers can be dropped in Slice 16.
- **Hubs migrated (Slices 14–15b):** sidebar visibility, data-params spine, view-layers hub (`viewLayersAtom`, `viewLayersParamsAtom`). **Still on Recoil until Slice 16:** `interactionGroupsState`, `terracottaColorMapValuesQuery` (context provider), `apiFeatureQuery`.
