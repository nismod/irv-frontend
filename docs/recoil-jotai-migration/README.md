...
| [`data/state-graph.json`](./data/state-graph.json)   | Source of truth for the graph: 152 nodes (atoms / selectors / families / boundary providers), 201 edges (typed `get`/`set`/`default`/`effect`/`syncReplica`/`urlSync`/`localStorageSync`/`routeSync`/`providerValue`), and 100 component-consumer pairs.                                                      |
...
- **Sync surface (highest-risk migration item):** one `RecoilLocalStorageSync` ([`src/lib/recoil/sync-stores/RecoilLocalStorageSync.tsx`](../../../src/lib/recoil/sync-stores/RecoilLocalStorageSync.tsx)), one `RecoilURLSyncJSON` (in [`src/App.tsx`](../../../src/App.tsx)), one route-scoped `RecoilSync` ([`src/pages/map/MapViewRouteSync.tsx`](../../../src/pages/map/MapViewRouteSync.tsx)). Replacing this layer is the single biggest design decision.
- **Hubs migrated (Slices 14–15b):** sidebar visibility, data-params spine, view-layers hub (`viewLayersAtom`, `viewLayersParamsAtom`). **Still on Recoil until Slice 16:** `interactionGroupsState`, `terracottaColorMapValuesQuery` (context provider), `apiFeatureQuery`, Downloads jobs local-storage atoms.
- **Easy wins to migrate first** (no inbound hub edges): Articles map (nested `RecoilRoot`), place search, mobile tab content flags, pixel-driller accordion, downloads jobs.
...
- **The actual migration code.** No `recoil` → `jotai` changes have been made; no Jotai dependency was added; no helper file under `src/lib/recoil/` was edited.
